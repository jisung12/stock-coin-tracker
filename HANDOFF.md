...
[ ] 3. (키 하드코딩이면) Cloudflare Secret으로 이전 + env 참조로 수정
       (이미 Secret이면 이 단계 건너뜀)
[ ] 4. (정석) 미장 등락률을 워커에서 직접 반환하도록 수정 → 아래 8번 섹션 참고
[ ] 5. 로컬/라이브 테스트: /stocks/us 가 change를 숫자로 주는지 + 미장 화면 정상인지
[ ] 6. 진짜 worker.js 기준으로 wrangler deploy (워커 배포)
[ ] 7. worker.js + wrangler.toml(크론/triggers 포함)을 커밋 → push
       # 이때 레포의 구버전 worker.js를 진짜 버전으로 덮어쓰게 됨 (정상)
[ ] 8. scheduled() 핸들러 점검: 크론이 매 1분인데 매번 실제로 한투를 때리는지,
...
회사에서 미장 버그는 (프론트 우회로) 고쳐서 라이브·GitHub 둘 다 반영했다. 집에서 할 일은 **(1) 이 PC에만 있는 진짜 worker.js를 한투 키 안전 처리 후 GitHub에 백업하고, (2) 미장 등락률을 정석대로 워커에서 처리하도록 고치는 것**이다.

---

## 8. 정석 작업: 미장 등락률(change)을 워커에서 처리 (근본 원인 수정)

### 배경 (왜 이게 정석인가)
- 미장이 안 나오던 진짜 원인은 **워커 `/stocks/us`가 등락률 `change`를 `null`로 주기 때문**이다. 국장 `/stocks/kr`는 `change`를 제대로 준다(예: `-12.31`). 미장만 빠져있다.
- 회사에서는 워커 소스가 없어서(이 PC에만 있음) 워커를 못 고쳤다. 그래서 **프론트(`script.js`)에서 임시 우회**를 넣었다: 워커가 같이 주는 전일종가 `base`로 `(price-base)/base*100`을 계산.
- 정석은 국장처럼 **워커가 `change`를 직접 계산해서 주는 것**이다. 그러면 미장/국장 응답 구조가 통일되고, 프론트는 표시만 하면 된다.

### worker.js에서 할 일
1. `/stocks/us` 응답을 만드는 부분(한투 해외주식 현재가 호출 결과를 가공해서 `{symb, price, change, base, ...}` 형태로 내보내는 곳)을 찾는다.
2. 등락률을 채운다. 우선순위:
   - 한투 해외주식 현재가 응답의 **등락율 필드(`rate`)** 를 `change`에 매핑.
   - `rate`가 비어있거나(장 마감 등) null이면 **전일종가로 계산**: `change = (현재가 - base) / base * 100`.
   - (왜 빠졌는지도 같이 확인: 기존 코드가 엉뚱한 필드명을 봤거나, base만 받고 등락률 매핑을 빼먹었을 가능성이 큼.)
3. 국장(`/stocks/kr`) 응답에서 change를 어떻게 채우는지 보고 같은 방식으로 맞추면 깔끔하다.

### 테스트
- `Invoke-RestMethod https://market-data-api.jysung1210.workers.dev/stocks/us` → 각 종목 `change`가 **null이 아니라 숫자**로 오는지 확인.
- 사이트(`stock-coin-tracker.pages.dev`) → 주식 → 미장 → 등락률 부호/색(빨강·초록) 정상인지 확인.
- 배포는 **진짜 worker.js 기준 `wrangler deploy`**. 프론트는 변경 없으면 재배포 불필요.

### 프론트 우회 코드는 그대로 둬도 됨 (중요)
회사에서 넣은 `script.js`의 등락률 계산은 이렇게 방어적으로 짜놨다:
```js
const chg = (typeof item.change === 'number')
  ? item.change                                   // 워커가 숫자로 주면 그걸 우선 사용
  : (item.base ? (item.price - item.base)/item.base*100 : null);  // null일 때만 계산(폴백)
```
→ 워커가 `change`를 제대로 주기 시작하면 **프론트는 자동으로 워커 값을 쓰고 계산식은 발동 안 한다.** 그래서 워커를 정석대로 고쳐도 충돌/중복 없음. 프론트 폴백은 안전망으로 남겨두면 된다(워커가 또 null 줄 때 화면 안 깨짐).

---

## 9. 상단 티커 스트립의 지수 연결 (집 작업, 프론트는 준비 완료)

회사에서 상단에 **가로 스크롤 티커 스트립**을 추가했다. 순서: `환율 · 비트코인 · 코스피 · 코스닥 · S&P500 · 나스닥 · 다우`. 모바일에서 줄바꿈 없이 가로 스크롤(안 무너짐) 확인 완료.

상태:
- **환율, 비트코인 = 실데이터 연결 완료**(프론트만으로 됨. 환율 exchangerate, BTC Binance).
- **지수 5개(코스피·코스닥·S&P500·나스닥·다우) = 현재 "-" 플레이스홀더.** 워커 데이터가 없어서 비워둠.

집에서 할 일:
1. 워커에 지수 데이터 엔드포인트 추가/연결.
   - 코스피·코스닥: 한투 **국내 업종지수 현재가** API
   - 미장 지수: 한투 **해외지수** API (또는 기존 SPY/QQQ/DIA ETF로 대체)
2. script.js에서 그 데이터로 갱신 헬퍼만 호출하면 칩이 채워진다. 헬퍼 이미 있음:
   ```js
   // key: 'kospi'|'kosdaq'|'sp500'|'nasdaq'|'dow', prefix 생략(원화/지수표기)
   updateTickerChip('kospi', 3205.42, 0.82);
   updateTickerChip('sp500', 6481.30, -1.21);
   ```
3. 칩 탭 → 차트: 비트코인은 이미 연결됨(`showDetail`). 지수 차트는 데이터 생기면 연결.

> 즉 프론트 골격(칩, 스타일, 갱신 헬퍼)은 다 깔아놨고, 집에서는 **워커에서 지수 값 받아 `updateTickerChip(...)` 호출 한 줄씩** 추가하면 끝이다.

---

## 10. 게시판 백엔드 (집 작업) — 프론트는 준비 완료

회사에서 **게시판 탭**(단일 자유게시판)을 추가했다. UI(글 목록 + 작성폼)는 다 만들었고, 글 저장/공유는 워커가 필요하다(데이터 영속화). 현재 라이브에선 "게시판 서버 준비 중입니다"로 표시된다.

프론트가 이미 아래 계약대로 호출 중이다 (`script.js`의 `fetchBoard`/`setupBoard`):
- **GET** `${WORKER_URL}/board` → `{ "success": true, "posts": [ { "name": "...", "text": "...", "ts": 1719200000000 } ] }` (최신순 권장)
- **POST** `${WORKER_URL}/board` (JSON body `{ "name": "...", "text": "..." }`) → `{ "success": true, "post": {...} }`

집에서 할 일:
1. 워커에 `/board` GET/POST 추가. 저장은 **KV 또는 D1**(뭐사지 D1 패턴 참고 가능).
2. 연결되면 게시판 탭의 "준비 중" 메시지가 자동으로 사라지고 목록/작성이 동작한다(프론트 수정 불필요).

주의 (공개 게시판이라 중요):
- **XSS**: 프론트는 출력 시 `escapeHtml`로 이스케이프했지만, 워커 저장 시에도 신뢰하지 말 것.
- **검증/제한**: name 20자, text 500자 제한(프론트에 maxlength 있음, 워커에서도 재검증). 빈 글 거부.
- **스팸/욕설/rate limit**: 공개라 봇/도배 우려. 간단한 IP rate limit이나 작성 간격 제한 권장.
